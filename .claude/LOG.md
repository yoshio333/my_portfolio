...
## 2026-09-26 About 受賞欄に みらい育成アワード2026 財団賞を追加
- 変更: `CONTENT.awards.items` の先頭に「みらい育成アワード2026　三菱みらい育成財団賞／みんなの探究LAB／運営・講座担当」を追加。`slug` を省略可能にし、無い行はリンクなしタグ（span）で出す（タグのスタイルは `awardTagStyle` に共通化）
- 理由: みんなの探究LAB は実績カードが無く `/work/<slug>` に飛ばせないため。探究アーカイブ（`/tankyu/`）は信用組合の活動で別物なので流用しない
- 次: 財団の公式発表ページが出たらリンクを貼る（ノブ指示）
